...
colorSlider.addEventListener("input", () => {
  ...
  const newColor = `hsl(${newColorValue}, 100%, 50%)`;
  colorBox.style.backgroundColor = newColor;
});

borderSlider.addEventListener("input", () => {
  const newBorderValue = borderSlider.value;
  const newBorder = `${newBorderValue}px`;
  colorBox.style.borderRadius = newBorder;
});

rotationSlider.addEventListener("input", () => {
  const newRotationValue = rotationSlider.value;
  const newRotation = `rotateZ(${newRotationValue}deg)`;
  colorBox.style.transform = newRotation;
});
